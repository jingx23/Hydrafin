...
Hydrafin offers two player options: **MPV** and **Native** (AVPlayer).

**MPV** (via [MPVKit](https://github.com/mpvkit/MPVKit)) replaces the VLCKit player used by upstream Swiftfin. Key advantages over VLCKit:

- **Broader codec support** — decodes TrueHD, DTS-HD, and more audio formats that VLCKit cannot handle
- **TLS 1.3** — VLCKit is limited to TLS 1.1/1.2; MPV uses the system TLS stack and supports 1.3
- **More accurate HDR tone mapping** — libplacebo-based pipeline with proper colorspace handling
- **No Airplay audio delay** — VLCKit has a [known Airplay audio delay bug](https://code.videolan.org/videolan/VLCKit/-/issues/544); MPV does not

**Native** (AVPlayer) is available for cases that benefit from Apple's native capabilities such as Picture-in-Picture and Framerate Matching.

All video, audio, and subtitle formats listed are supported for direct playback but may be repackaged based on container support. If transcoding is enabled on your server, any unsupported formats will be converted automatically.
...
| Audio Codec                                                                    | MPV | Native (AVPlayer) |
|--------------------------------------------------------------------------------|-----|-------------------|
...
| Video Codec                                                              | MPV    | Native (AVPlayer) |
|--------------------------------------------------------------------------|--------|-------------------|
...
| [H.266/VVC](https://en.wikipedia.org/wiki/Versatile_Video_Coding)        | ❌     | ❌                |
...
- [2] HEVC requires A8X Pro, M1, or newer for acceptable performance. Older devices that do not report HEVC capabilities have HEVC disabled by default. All devices supported by Hydrafin should have HEVC available.
...
| Subtitle Format                                                                 | MPV | Native (AVPlayer) |
|---------------------------------------------------------------------------------|-----|-------------------|
...
Track selection compatibility for each player. In testing, as of Hydrafin 1.0, the following interactions have been tested.
...
| External Audio + Internal Audio + External Subtitles                      | ✅         | ✅        | Cannot play external audio track if transcoding is required |
| External Audio + Internal Audio + Internal Subtitles                      | ✅         | ✅        | Cannot play external audio track if transcoding is required |
| External Audio + Internal Audio + Internal Subtitles + External Subtitles | ✅         | ✅        | Cannot play external audio track if transcoding is required |
...
|---------------------------------------------------------------------------|------------|-----------|-------|
| Internal Audio                                                            | ✅         | ✅        |       |
| Internal Audio + Internal Subtitles                                       | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| Internal Audio + External Subtitles                                       | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| Internal Audio + Internal Subtitles + External Subtitles                  | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| Multiple Internal Audio + Multiple Internal Subtitles                     | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| Multiple Internal Audio + Multiple External Subtitles                     | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| Multiple Internal Audio + Internal Subtitles + External Subtitles         | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| External Audio + Internal Audio + External Subtitles                      | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| External Audio + Internal Audio + Internal Subtitles                      | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |
| External Audio + Internal Audio + Internal Subtitles + External Subtitles | 🔶         | ❌        | Default audio track only; subtitles cannot be selected |

---

### Miscellaneous

| Feature                      | MPV    | Native (AVPlayer) | Notes |
|------------------------------|--------|-------------------|-------|
| **External Display Support** | 🔶     | ✅                | MPV can only be mirrored; player retains source device dimensions |
| **Energy Consumption**       | 🔶     | ✅                | MPV uses a software decoder for formats not handled by iOS hardware, resulting in higher power consumption |
